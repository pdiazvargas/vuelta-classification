...
7. ~~First deploy~~ — done. Plugin id **443984**, dashboard at https://trmnl.com/plugin_settings/443984/edit. `src/settings.yml`'s `id` and the workflow's `trmnlp push --id 443984` are filled in, and `push` now runs on merges to `main` (gated behind `lint`). One loose end: the CI `push` job needs a `TRMNL_API_KEY` secret on the `vuelta-classification` GitHub repo, which hasn't been set yet — add it with `gh secret set TRMNL_API_KEY --repo pdiazvargas/vuelta-classification` before merging anything that should auto-deploy. Also **remember to add the plugin to a device playlist** (https://trmnl.com/playlists) — pushing a plugin doesn't put it on a screen by itself.
